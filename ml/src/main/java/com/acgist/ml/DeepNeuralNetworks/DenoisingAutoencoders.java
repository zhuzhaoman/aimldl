package com.acgist.ml.DeepNeuralNetworks;

import static com.acgist.ml.util.ActivationFunction.sigmoid;
import static com.acgist.ml.util.RandomGenerator.binomial;
import static com.acgist.ml.util.RandomGenerator.uniform;
	...
	public static void main(String[] args) {
		...
		for (int pattern = 0; pattern < patterns; pattern++) {
			for (int n = 0; n < train_N_each; n++) {
				...
				for (int i = 0; i < nVisible; i++) {
					if ((n_ >= train_N_each * pattern && n_ < train_N_each * (pattern + 1)) && (i >= nVisible_each * pattern && i < nVisible_each * (pattern + 1))) {
			...
			for (int n = 0; n < test_N_each; n++) {
				...
				for (int i = 0; i < nVisible; i++) {
					if ((n_ >= test_N_each * pattern && n_ < test_N_each * (pattern + 1)) && (i >= nVisible_each * pattern && i < nVisible_each * (pattern + 1))) {
